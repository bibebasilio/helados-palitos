...
function cargarProductosCarrito() {
  ...
  if (carrito.length === 0) {
    tabla.innerHTML =
      '<tr><td colspan="8" style="text-align: center; padding: 20px;">Tu carrito está vacío.</td></tr>';
  } else {
  ...
  document.querySelectorAll(".cantidad-producto").forEach((input) => {
    input.onchange = (e) => {
      ...
      const producto = carrito.find(
        (p) => String(p.id) === String(e.target.id),
      );
      if (producto) {
...
function actualizarTotalCarrito(subtotal) {
  document
    ...
    .forEach((el) => (el.innerText = subtotal.toFixed(2)));
}

function recalcularTodo() {
  const totalBase =
    parseFloat(document.getElementById("total")?.innerText) || 0;
  const cuponInput =
    document.getElementById("coupon-input")?.value.trim().toUpperCase() || "";
  const metodoPagoCheck = document.querySelector(
    'input[name="metodo-pago"]:checked',
  );
  ...
  let costoEnvio =
    metodoPago === "efectivo" || !metodoPago ? 0 : totalBase * 0.23;
...
async function enviarPedidoWhatsApp() {
  alert(
    " Boton de finalizar compra presionado. Validando datos y preparando pedido...",
  );
  const faltantes = validarFormulario();
  if (faltantes.length > 0) {
    return alert(
      "No puedes finalizar el pedido. Falta completar: " + faltantes.join(", "),
    );
  }
  ...
  const telefono = document.getElementById("telefono")?.value.trim();
  const comentario =
    document.getElementById("comentario")?.value.trim() || "Sin comentarios";
  const finalTotalText =
    document.getElementById("final-total")?.innerText || "$0";

  const subtotal = document.getElementById("total")?.innerText || "0";
  const descCupon =
    document.getElementById("des-cupon")?.innerText.replace("-$", "") || "0";
  const descEfectivo =
    document.getElementById("des-efectivo")?.innerText.replace("-$", "") || "0";
  const costoEnvio =
    document.getElementById("shipping-cost")?.innerText.replace("$", "") || "0";
  const metodoPagoCheck = document.querySelector(
    'input[name="metodo-pago"]:checked',
  );
