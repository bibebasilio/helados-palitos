...
// 1. Configuración global de cupones
...
function validarFormulario(){
    ...
    const faltantes = validarCampos(nombre, direccion, telefono, comentario, metodoPago, valorFinalNum);
...
function actualizarEstadoBoton(boton, faltantes, mensajeError) {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Vinculamos el botón de finalizar a la función asíncrona
...
async function enviarPedidoWhatsApp() {
    ...
    try {
        // URL local de desarrollo para la comunicación con Express
        const servidorBase = "http://localhost:3000";

        // 1. Enviamos el pedido base a MongoDB Atlas para persistencia e impresión
        ...
        if (resultadoPedido.success) {
            ...
            // 2. PASO BLINDADO: Intento de descuento de stock en segundo plano
            try {
                ...
                if (resultadoStock.success) {
                    console.log("✅ Stock actualizado correctamente en MongoDB.");
                } else {
                    console.warn("⚠️ El servidor respondió pero no pudo procesar el stock:", resultadoStock.error);
                }
            } catch (stockError) {
                // Si la ruta llegara a fallar por un 404 transitorio, cae acá sin romper el flujo de WhatsApp
                console.error("❌ Error no crítico al descontar stock. Continuando con el pedido:", stockError);
            }

            // 3. Formateo del mensaje para la API de WhatsApp
